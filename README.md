...
**② 对方用豆包手机 App 端（实测推荐路径：创建技能）**，让 ta 按以下步骤操作（截图实测路径）：

1. 豆包 App 底部把对话模式切到 **「工作」**
2. 在输入框上方的功能按钮里点 **「创建技能」**
3. 输入「帮我创建技能：」+ 粘贴 [`SKILL.md`](SKILL.md) 中 frontmatter 之后的**全部正文**，发送
4. 豆包自动生成技能后，直接发照片即可触发工作流（诊断 → 方案 → 执行 → 复盘）

> 手机 App 无安装 GitHub 技能的能力，"读文档"方式只能临时生效一次；**创建技能是手机端唯一持久化方式**。App 也可用读文档话术应急：
>
> ```
