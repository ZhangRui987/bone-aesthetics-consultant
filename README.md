...
1. 打开豆包 App，点**左上角「≡」菜单** → 选择 **「技能与连接器」**
2. 进入后点**右上角「＋」**→ 选创建技能
3. 把 [`SKILL.md`](SKILL.md) 中 frontmatter 之后的**全部正文**粘贴进去，保存
4. 直接发照片即可触发工作流（诊断 → 方案 → 执行 → 复盘）

> 备选入口：底部对话模式切到「工作」→ 输入框上方点「创建技能」→ 发送"帮我创建技能：" + 粘贴正文，同样可行。
